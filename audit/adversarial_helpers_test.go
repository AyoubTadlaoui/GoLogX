...
func craftEntry(t *testing.T, seq uint64, prev [hashLen]byte, ts string, data json.RawMessage, signer ed25519.PrivateKey) (line []byte, sum [hashLen]byte) {
	t.Helper()
...
// bytesReplaceOnce replaces the first occurrence of old with repl and fails the
...
func bytesReplaceOnce(t *testing.T, b []byte, old, repl string) []byte {
	t.Helper()
	out := bytes.Replace(b, []byte(old), []byte(repl), 1)
